...
**採用モデル: `gpt-5.2-nano`（一次判定） + `gpt-5.2`（重要時の二次精査）**

GPT-5.2はOpenAIが2025年12月11日にリリースした3バリアント構成のモデル。

| バリアント                | 用途                                                          | コスト感              |
| ------------------------- | ------------------------------------------------------------- | --------------------- |
| **gpt-5.2-nano** | 差分検知の一次判定（常時実行） | 最低 |
...
LLM_MODEL_DIFF = config["llm"]["model_diff"]      # "gpt-5.2-nano"
...
STEP 7: call_gpt = True の場合、まず gpt-5.2-nano で一次判定

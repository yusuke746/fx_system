...
| LightGBM        | 32特徴量（MTF SMC版）→ 上昇/横ばい/下落 確率出力                                   | LightGBM 4.x   |
| エグジット      | ATR動的SL / スケールアウト / ドテン高速連続発注（インターバル制限付き）/ 時間ベース | Python         |
...
毎1分:   ポジション監視（スケールアウトStep確認・トレーリング更新・時間Exitチェック）

毎日深夜01:00 JST:  DBメンテ・バックアップ・カレンダーキャッシュ更新

毎週土曜14:00 JST:  週末自動最適化ループ（28日最適化 + 14日アウトオブサンプル検証の二段階構え）
...
## 3. GPT-5.2モジュール設計（差分検知）

### 3.1 採用モデルの選定

**採用モデル: `gpt-5.2-instant`（差分検知・通常時のデフォルト）**

GPT-5.2はOpenAIが2025年12月11日にリリースした3バリアント構成のモデル。

| バリアント                | 用途                                                          | コスト感              |
| ------------------------- | ------------------------------------------------------------- | --------------------- |
| **gpt-5.2-instant** | 差分検知・通常の判定タスク ←**本システムのデフォルト** | 低（$1.75/Mトークン） |
...
LLM_MODEL = config["llm"]["model_instant"]   # "gpt-5.2-instant"
```
...
STEP 7: call_gpt = True の場合のみ GPT-5.2 API呼び出し
```
...
TP_MULTIPLIER = 2.5   # ATR × 2.5
SL_TP_MIN_PIPS = 15
SL_TP_MAX_PIPS = 35
```

### 4.2 32特徴量リスト（MTF SMC版）
...
- `spread_pips` — 現在スプレッド（pips）
- `session_flag` — セッション（London=1, NY=2, Overlap=3, Other=0）
- `hour_of_day` — JST時刻（時間帯特徴）
- `day_of_week` — 曜日（0=月曜〜4=金曜）
...
### 5.1 エグジット優先順位（v2.2最終確定版）

| 優先度              | 種別                      | 詳細・実装                                                                              |
| ------------------- | ------------------------- | --------------------------------------------------------------------------------------- |
| **1（最高）** | Calendar Veto強制クローズ | Layer A Veto発動中（重要指標30分前）→ 全ポジション強制クローズ・SLをBEへ移動           |
| **2**         | ATR動的SL到達             | MT5のOCO注文として設定済み（SL=ATR×1.5）。MT5が自動執行                                |
| **3**         | 反対シグナルドテン        | LightGBM逆方向>65% + インターバル確認（1時間以内なら決済のみ・新規禁止）→ 高速連続発注 |
| **4**         | スケールアウト Step1      | +1.0×ATR達成 → 50%利確（比率は週末最適化で変動）+ SLをBEへ移動                        |
| **5**         | スケールアウト Step2      | +1.8×ATR達成 → 追加25%利確 + 残25%をトレーリングへ移行                                |
| **6**         | トレーリングストップ      | 残ポジ: 高値更新ごとに0.8×ATR追従                                                      |
| **7**         | 時間ベース強制クローズ    | 4時間超過 / 金曜22:00 / セッション跨ぎ → 残ポジ全決済                                  |
...
CREATE TABLE trades (
    ...
    pnl_pips    REAL,
    pnl_jpy     REAL,
    exit_reason TEXT,                  -- 'atr_sl'|'scale_out_1'|'doten'|'time_exit'|'calendar_veto'
...
CREATE TABLE api_call_log (
    ...
    model       TEXT NOT NULL,         -- 'gpt-5.2-instant'
    tokens_in   INTEGER,
    tokens_out  INTEGER,
    cost_usd    REAL,
    created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP
...
### 7.2 週末自動最適化ループ（二段階構え版）

**最適化対象:** エグジットパラメータのみ（LightGBMモデル自体は対象外）
...
STEP 1: 直近28日のクローズ済みトレードを取得
        → サンプル数が30件未満の場合はスキップ（前週パラメータを継続）+ Discord通知

STEP 2: グリッドサーチでパラメータ組み合わせを評価
        評価指標: Sharpe比（主軸）× 最大DD（ペナルティ）の複合スコア
        探索空間:
          step1_ratio: [0.30, 0.40, 0.50, 0.60, 0.70]
          step2_ratio: [0.15, 0.20, 0.25, 0.30, 0.35]
          sl_mult:     [1.2, 1.3, 1.5, 1.7, 2.0]
          tp_mult:     [2.0, 2.2, 2.5, 2.8, 3.0, 3.5]
        合計: 5×5×5×6 = 750通り（数秒で完了）

STEP 3: 安全チェック — 前週比±20%以内に制限

=== PHASE B: アウトオブサンプル検証（直近14日で確認） ===

STEP 4: PHASE Aとは独立した直近14日のトレードで候補パラメータを評価
        → 直近14日のSharpe比が前週パラメータを下回る場合: 変更を取り消し前週継続
        → 上回る場合: STEP 5へ進む

STEP 5: 新しいパラメータを config.json に書き込み（日曜23:00の再学習より前に完了）

STEP 6: Discordへレポート送信
        ・最適化期間（28日）Sharpe比 / 検証期間（14日）Sharpe比
        ・変更前→変更後のパラメータ対比 / 適用の可否と理由
```
...
| 安全装置               | 内容                                                                 |
| ---------------------- | -------------------------------------------------------------------- |
| サンプル数チェック     | 30件未満はスキップ                                                   |
| 変化量制限             | 前週比±20%以内                                                      |
| アウトオブサンプル検証 | 直近14日（最適化期間外）で候補パラメータを事前検証                   |
| 週次ロールバック       | 適用翌週に勝率が前週比-10%以上悪化した場合、前週パラメータに自動復元 |
| 手動オーバーライド     | `config.json` の `auto_optimize: false` でいつでも無効化         |
...
| 毎週土曜 | **14:00** | 週末自動最適化ループ（28日最適化 + 14日アウトオブサンプル検証）→ config.json更新 → Discord送信                                                                  |
| 毎週日曜 | 23:00           | LightGBM再学習（直近90日）/ ウォークフォワード検証 / PSIドリフト検知 / signalsアーカイブ移動 / VACUUM ANALYZE / api_call_log週次集計 / 週次レポートDiscord送信    |
...
| LLM              | OpenAI GPT-5.2 Instant（モデル名はconfig.jsonで管理）     | 確定                                |
...
| 自動最適化       | gridsearch（内製）+ config.json自動書き込み               | v2.2新規追加                        |
...
```json
{
  ...
  "llm": {
    "model_instant": "gpt-5.2-instant",
    "model_thinking": "gpt-5.2-thinking",
    "llm_enabled": true,
  ...
  "risk": {
    ...
    "sl_min_pips": 15,
    "sl_max_pips": 35,
    "scale_out_step1_ratio": 0.50,
    "scale_out_step2_ratio": 0.25,
...
def calc_lot_size(
    ...
    例: 残高100万円 / SL=20pips / USDJPY
      ...
      → 1pip = 0.01lot × 100 = 10円 (USDJPY mini lot)
      → lot = 20,000 / (20 × 1,000) = 1.0 lot
    """
    pip_values = {
